...
import { FIND_DETAILED_TRAINEE_RESULT_URL} from "../../constants/apiConstants";
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        ...
        const response = await axios.post(`${FIND_DETAILED_TRAINEE_RESULT_URL}`, {
    ...
    for (let i = 1; i <= 8; i++) {
      ...
      if (question[optionKey]) {
        options.push({
          ...
          isChosen: chosenOption === optionKey || 
    ...
    : "Incorrect"}
                  ...
                  <div className={styles.questionFooter}>
                    <span className={styles.questionType}>
                      Type: {item.questionDto.questionType === 'single_choice' ? 'Single Choice' : 'Multiple Choice'}
                    </span>
                  </div>
                </div>
              ))
